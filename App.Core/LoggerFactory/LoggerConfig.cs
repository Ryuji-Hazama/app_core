...
public class LoggerConfig : ILoggerConfig
{
    ...
    public LoggerConfig()
    {
        LoadLoggerConfig();
        if (Json_Helper == null)
            Json_Helper = new Json.JsonHelper();
    }
    ...
    private Containers.ConfigurationFile LoadConfigFromJson(string log_config_json)
    {
        ...
        Json_Helper = new Json.JsonHelper(log_config_json);
        var logger_config = Json_Helper.DeserializeJsonContent<Containers.ConfigurationFile>();
    ...
    private OutputType ObjectToOutputType(object output_type)
    {
        try
        {
            output_type = Json_Helper.UnwrapJsonValue(output_type) ?? throw new ArgumentNullException(nameof(output_type), "Output type cannot be null.");
    ...
    private LogFileMode ObjectToLogFileMode(object log_file_mode)
    {
        try
        {
            log_file_mode = Json_Helper.UnwrapJsonValue(log_file_mode) ?? throw new ArgumentNullException(nameof(log_file_mode), "Log file mode cannot be null.");
    ...
    private int ObjectToMaxFileSize(object max_file_size)
    {
        try
        {
            max_file_size = Json_Helper.UnwrapJsonValue(max_file_size) ?? throw new ArgumentNullException(nameof(max_file_size), "Max file size cannot be null.");
    ...
    public LogLevel ObjectToLogLevel(object log_level)
    {
        try
        {
            log_level = Json_Helper.UnwrapJsonValue(log_level) ?? throw new ArgumentNullException(nameof(log_level), "Log level cannot be null.");
            object log_level_type = log_level.GetType() ?? typeof(object);
